class GoDaddyDns
  ...
  def self.update_txt domain, subdomain, value
    ...
    url = "https://api.godaddy.com/v1/domains/#{domain}/records/TXT"
    payload = [
      {
        "data" => value,
        "name" => subdomain,
  ...
  def self.update_dev_dylanjones_acme value
    return update_txt "anjon.es", "_acme-challenge.dev.dyl", value
  end

  def self.update_dylanjones_acme value
    return update_txt "anjon.es", "_acme-challenge.dyl", value
  end
